...
# Help
if [[ $1 == *"help"* ]] ; then
  COMMAND="curl -s https://get.codex.storage/run.sh"
  echo -e "
  \e[33mRun Codex\e[0m\n
  \e[33mUsage:\e[0m
    ${COMMAND} | bash
...
if ! command -v ${CODEX_BINARY} &> /dev/null; then
  show_fail "Checking if Codex is installed" "Please install Codex first by running 'curl -s https://get.codex.storage/install.sh | bash'"
fi
show_pass "${message}"
...
if [[ ! -f ${CODEX_ETH_PRIVATE_KEY} ]]; then
  show_fail "Checking private key" "Please generate private key by running 'curl -s https://get.codex.storage/generate.sh | bash'
...
case ${NETWORK} in
  ...
  workshop)
    ...
    export CODEX_ETH_PROVIDER="https://rpc.testnet.codex.storage"
    ;;
  *) show_fail "Unknown network" "Please set the NETWORK environment variable to 'testnet'."
    ;;
esac
